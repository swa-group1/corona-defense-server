...
namespace BackEnd.Game.Systems
{
  ...
  internal class PlaceTowerSystem : IEcsSystem
  {
    ...
    private readonly Dictionary<int, TowerType> towers;
    ...
    {
      this.towers = new Dictionary<int, TowerType>();
      foreach (TowerType tower in towerDefinitions.Towers)
      {
    ...
    {
      if (!this.towers.TryGetValue(request.TowerTypeNumber, out TowerType towerDefinition))
      {
        return;
      }
